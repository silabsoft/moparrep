...
// MoparRep: optional setting to unlock wallet for block minting only;
...
// MoparRep: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // MoparRep: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // MoparRep: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // MoparRep: not avatar mode
                    {
...
// MoparRep: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew, int64 nMoneySupply)
{
    ...
    if (mapArgs.count("-primenodekey") && mapArgs.count("-primenoderate")) // MoparRep: primenode priv key
    {
...
// MoparRep: check 'spent' consistency between wallet and txindex
// MoparRep: fix wallet spent state according to txindex
...
// MoparRep: disable transaction (only for coinstake)
